...
All three subagents are instructed to prefer **Serena MCP tools** (`mcp__plugin_config-doctor_serena__*`) over native file tools for efficiency. Serena is declared in `.mcp.json` and launched via `uvx` from `git+https://github.com/oraios/serena`. The `uv` package manager is a prerequisite.
